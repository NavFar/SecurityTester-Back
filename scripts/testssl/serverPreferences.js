var serverPreferences= function(job,done){
    ...
    var keys = [ 'serverPreferences'];
    var fileName = job.data.url+job.data.id+"serverPreferences";
    ...
    filePath = path.join(filePath,"testssl.sh");
     exec(filePath+" --quiet --warnings=batch --preference  --jsonfile-pretty "+fileName+" "+job.data.url, (error, stdout, stderr) => {
       if(error | stderr)
       {
