var browserSimulations= function(job,done){
    ...
    var keys = [ 'browserSimulations'];
    var fileName = job.data.id+"browserSimulations";
    ...
    filePath = path.join(filePath,"testssl.sh");
     exec(filePath+" --quiet --warnings=batch --client-simulation --jsonfile-pretty "+fileName+" "+job.data.url, (error, stdout, stderr) => {
       if(error | stderr)
       {
