var protocols= function(job,done){
    ...
    var keys = [ 'protocols'];
    var fileName = job.data.url+job.data.id+"protocols";
    ...
    filePath = path.join(filePath,"testssl.sh");
     exec(filePath+" --quiet --warnings=batch --protocols  --jsonfile-pretty "+fileName+" "+job.data.url, (error, stdout, stderr) => {
       if(error | stderr)
       {
