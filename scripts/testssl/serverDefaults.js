var serverDefaults= function(job,done){
    ...
    var keys = [ 'serverDefaults'];
    var fileName = job.data.url+job.data.id+"serverDefaults";
    ...
    filePath = path.join(filePath,"testssl.sh");
     exec(filePath+" --quiet --warnings=batch --server_defaults  --jsonfile-pretty "+fileName+" "+job.data.url, (error, stdout, stderr) => {
       if(error | stderr)
       {
