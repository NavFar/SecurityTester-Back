var ciphers= function(job,done){
    ...
    var keys = [ 'ciphers'];
    var fileName = job.data.url+job.data.id+"ciphers";
    ...
    filePath = path.join(filePath,"testssl.sh");
     exec(filePath+" --quiet --warnings=batch --each-cipher  --jsonfile-pretty "+fileName+" "+job.data.url, (error, stdout, stderr) => {
       if(error | stderr)
       {
